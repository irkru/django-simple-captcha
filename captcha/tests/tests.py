...
class CaptchaCase(TestCase):
    ...
    def setUp(self):
        ...
        self.stores = {}
        self.__current_settings_output_format = settings.CAPTCHA['OUTPUT_FORMAT']
        self.__current_settings_dictionary = settings.CAPTCHA['WORDS_DICTIONARY']
        self.__current_settings_punctuation = settings.CAPTCHA['PUNCTUATION']
        ...
        if os.path.exists('/usr/share/dict/words'):
            settings.CAPTCHA['WORDS_DICTIONARY'] = '/usr/share/dict/words'
            settings.CAPTCHA['PUNCTUATION'] = ';-,.'
    ...
    def tearDown(self):
        settings.CAPTCHA['OUTPUT_FORMAT'] = self.__current_settings_output_format
        settings.CAPTCHA['WORDS_DICTIONARY'] = self.__current_settings_dictionary
        settings.CAPTCHA['PUNCTUATION'] = self.__current_settings_punctuation
    ...
    def testRepeatedChallengeFormSubmit(self):
        __current_challange_function = settings.CAPTCHA['CHALLENGE_FUNCT']
        for urlname in ('captcha-test', 'captcha-test-model-form'):
        ...
        settings.CAPTCHA['CHALLENGE_FUNCT'] = __current_challange_function
    ...
    def testTestMode_Issue15(self):
        __current_test_mode_setting  = settings.CAPTCHA['CAPTCHA_TEST_MODE']
        settings.CAPTCHA['CAPTCHA_TEST_MODE'] = False
        r = self.client.get(reverse('captcha-test'))
        self.assertEqual(r.status_code, 200)
        r = self.client.post(reverse('captcha-test'), dict(captcha_0='abc', captcha_1='wrong response', subject='xxx', sender='asasd@asdasd.com'))
        self.assertFormError(r, 'form', 'captcha', ugettext_lazy('Invalid CAPTCHA'))

        settings.CAPTCHA['CAPTCHA_TEST_MODE'] = True
        ...
        self.assertTrue(str(r.content).find('Form validated') > 0)
        settings.CAPTCHA['CAPTCHA_TEST_MODE'] = __current_test_mode_setting
