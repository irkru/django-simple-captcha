...
def random_char_challenge():
    ...
    for i in range(settings.CAPTCHA['LENGTH']):
...
def word_challenge():
    fd = file(settings.CAPTCHA['WORDS_DICTIONARY'], 'rb')
    l = fd.readlines()
    fd.close()
    while True:
        word = random.choice(l).strip()
        if len(word) >= settings.CAPTCHA['DICTIONARY_MIN_LENGTH'] and len(word) <= settings.CAPTCHA['DICTIONARY_MAX_LENGTH']:
            break
...
def huge_words_and_punctuation_challenge():
    ...
    while True:
        ...
        punct = random.choice(settings.CAPTCHA['PUNCTUATION'])
...
def noise_arcs(draw, image):
    size = image.size
    draw.arc([-20, -20, size[0], 20], 0, 295, fill=settings.CAPTCHA['FOREGROUND_COLOR'])
    draw.line([-20, 20, size[0] + 20, size[1] - 20], fill=settings.CAPTCHA['FOREGROUND_COLOR'])
    draw.line([-20, 0, size[0] + 20, size[1]], fill=settings.CAPTCHA['FOREGROUND_COLOR'])
    return draw
...
def noise_dots(draw, image):
    ...
    for p in range(int(size[0] * size[1] * 0.1)):
        draw.point((random.randint(0, size[0]), random.randint(0, size[1])), fill=settings.CAPTCHA['FOREGROUND_COLOR'])
    return draw
